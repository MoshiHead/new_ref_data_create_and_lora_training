...
class InferenceAgent:
    def __init__(self, opt):
        ...
        self.opt = opt
        self.rank = opt.rank
        self._blink_maps = None
        self._eye_masks = None
        self._blink_source_path = None
    ...
    def _extract_motion_tensor_from_payload(self, payload, path: str) -> torch.Tensor:
        if isinstance(payload, torch.Tensor):
            ...
            if motion is None:
                tensor_items = [
                    ...
                    if isinstance(value, torch.Tensor)
                    and value.ndim >= 2
                    and value.shape[-1] == int(self.opt.dim_w)
                ]
                if tensor_items:
        ...
        if motion.ndim == 3 and motion.shape[0] == 1:
            motion = motion[0]
        if motion.ndim != 2 or motion.shape[-1] != int(self.opt.dim_w):
            raise ValueError(
                f"Blink motion must have shape (T,{int(self.opt.dim_w)}) or "
                f"(1,T,{int(self.opt.dim_w)}); got {tuple(motion.shape)} from {path}"
            )
        return motion.contiguous()

    def _make_eye_mask(self, height: int, width: int, device, dtype) -> torch.Tensor:
        yy = torch.linspace(0.0, 1.0, int(height), device=device, dtype=dtype).view(1, 1, height, 1)
        xx = torch.linspace(0.0, 1.0, int(width), device=device, dtype=dtype).view(1, 1, 1, width)
        center_y = float(getattr(self.opt, "eye_center_y", 0.405))
        radius_x = max(float(getattr(self.opt, "eye_radius_x", 0.145)), 1e-6)
        radius_y = max(float(getattr(self.opt, "eye_radius_y", 0.070)), 1e-6)
        feather = max(float(getattr(self.opt, "eye_feather", 0.10)), 1e-6)

        mask = torch.zeros(1, 1, height, width, device=device, dtype=dtype)
        for center_x in (
            float(getattr(self.opt, "eye_left_x", 0.36)),
            float(getattr(self.opt, "eye_right_x", 0.64)),
        ):
    ...
    @torch.no_grad()
    def _init_eye_blink_composite(self, g_r, device, dtype) -> None:
        blink_path = str(getattr(self.opt, "blink_motion_path", "") or "")
        if not blink_path:
        ...
        if self._blink_maps is not None and self._blink_source_path == blink_path:
            return

        payload = torch.load(blink_path, map_location="cpu")
        blink_motion = self._extract_motion_tensor_from_payload(payload, blink_path)
        blink_maps_parts = []
        chunk = max(1, int(getattr(self.opt, "render_sub_batch", 8)))
        for start in range(0, int(blink_motion.shape[0]), chunk):
            sub = blink_motion[start:start + chunk].to(device, dtype=dtype)
            g_sub = g_r.expand(int(sub.shape[0]), -1)
            ta_b = self.ae.adapt(sub, g_sub)
            maps_b = self.ae.latent_token_decoder(ta_b)
        ...
        self._eye_masks = tuple(
            self._make_eye_mask(m.shape[-2], m.shape[-1], device=device, dtype=dtype)
            for m in self._blink_maps
        )
        self._blink_source_path = blink_path
        shapes = [tuple(m.shape) for m in self._blink_maps]
        print(
            f"[generate][blink] cached blink maps from {blink_path}: "
            f"frames={int(blink_motion.shape[0])} shapes={shapes}",
            flush=True,
        )

    def _composite_eye_blink_maps(self, current_maps, frame_idx: int):
        if self._blink_maps is None or self._eye_masks is None:
            return current_maps
        blink_len = int(self._blink_maps[0].shape[0])
        if blink_len <= 0:
            return current_maps
        idx = int(frame_idx) % blink_len
        composited = []
        for cur, blink_all, mask in zip(current_maps, self._blink_maps, self._eye_masks):
            blink = blink_all[idx:idx + 1].to(device=cur.device, dtype=cur.dtype)
    ...
    def decode_image(self, f_r, t_r, t_c, g_r):
        ...
        if bool(getattr(self.opt, "enable_eye_blink_composite", False)):
            self._init_eye_blink_composite(g_r, device=t_c.device, dtype=t_c.dtype)
        ...
        for t in range(T):
            ...
            if bool(getattr(self.opt, "enable_eye_blink_composite", False)):
                m_c = self._composite_eye_blink_maps(m_c, t)
...
class InferenceOptions(BaseOptions):
    def initialize(self, parser):
        ...
        parser.add_argument("--blink_motion_path", type=str, default="", help="Cached blink motion latent .pt file.")
