...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, seq_len=None, start_pos=0):
        ...
        cache_len = seq_len + start_pos
        if cache_len > self.max_seq_len_cached:
            self._set_cos_sin_cache(seq_len=cache_len, device=x.device, dtype=x.dtype)
        return (
            self.cos_cached[start_pos : start_pos + seq_len].to(dtype=x.dtype),
            self.sin_cached[start_pos : start_pos + seq_len].to(dtype=x.dtype),
        )
        ...
        if self.fused_attn:
            ...
            if causal:
                causal_mask = torch.triu(
                    torch.ones(
                        N, N, dtype=torch.bool, device=attn.device
                    ),
                    diagonal=1,
                )
                attn = attn.masked_fill(
                    causal_mask.view(1, 1, N, N),
...
class FlowMatchingTransformer(nn.Module):
    ...
    def sequence_embedder(
        ...
        if train:
            ...
            sequence = sequence.masked_fill(drop_mask, 0)
        return sequence
    ...
    def forward(
        ...
        **kwargs
        ...
        if prev_x is not None:
            ...
            if getattr(self.opt, "lookahead_frames", 0) > 0:
                L = self.opt.lookahead_frames
                a = torch.cat([a[:, L:, :], a[:, -1:, :].repeat(1, L, 1)], dim=1)
        ...
        # Calculate RoPE. In streaming/chunked sampling, `global_offset`
        # keeps the history frames and current frames in the same absolute
        # coordinate system instead of resetting RoPE to zero every chunk.
        rotary_pos_emb = self.rotary_emb(
            x,
            seq_len=x.shape[1],
            start_pos=kwargs.get("global_offset", 0),
        )
        ...
        for block in self.blocks:
            x = block(
                ...
                c,
                rotary_pos_emb=rotary_pos_emb,
                causal=getattr(self.opt, "causal_mask", False),
            )
        ...
        **kwargs
        ...
        if a_cfg_scale != 1.0:
            ...
            model_output = self.forward(
                ...
                train=False,
                global_offset=kwargs.get("global_offset", 0),
            )
