...
class FMGenerator(nn.Module):
    ...
    def sample(
        ...
        if isinstance(noise_init, torch.Tensor):
            ...
            noise_init = None
        noise_temporal_corr = float(
            data.get(
                "noise_temporal_corr",
                getattr(self.opt, "noise_temporal_corr", 0.0),
            )
            or 0.0
        )
        noise_temporal_corr = max(0.0, min(0.999, noise_temporal_corr))
        motion_prior_noise_blend = float(
            data.get(
                "motion_prior_noise_blend",
                getattr(self.opt, "motion_prior_noise_blend", 0.0),
            )
            or 0.0
        )
        motion_prior_noise_blend = max(0.0, min(1.0, motion_prior_noise_blend))
        disable_rope_global_offset = bool(
            data.get(
                "disable_rope_global_offset",
                getattr(self.opt, "disable_rope_global_offset", False),
            )
        )
        ...
        for chunk_idx in range(num_chunks):
            ...
            if isinstance(noise_init, torch.Tensor):
                ...
                if x0.shape[1] < self.num_frames_for_clip:
                    ...
                    pad = torch.randn(B, pad_len, self.opt.dim_w, device=device)
            ...
            if prev_x0_ctx is not None and prev_x0_ctx.numel() > 0:
                ...
                if carry_frames > 0:
                    x0[:, :carry_frames, :] = (
                        prev_x0_ctx[:, -carry_frames:, :]
                        .to(device=x0.device, dtype=x0.dtype)
                        .clone()
                    )
            if noise_temporal_corr > 0.0:
                fresh_start = max(1, int(carry_frames))
                innovation_scale = float((1.0 - noise_temporal_corr ** 2) ** 0.5)
                for frame_idx in range(fresh_start, x0.shape[1]):
                    x0[:, frame_idx, :] = (
                        noise_temporal_corr * x0[:, frame_idx - 1, :]
                        + innovation_scale * x0[:, frame_idx, :]
                    )
            if motion_prior_noise_blend > 0.0:
                if prev_sample_t is not None and prev_sample_t.numel() > 0:
                    last_motion = prev_sample_t[:, -1:, :].to(device=device, dtype=x0.dtype)
                    if prev_sample_t.shape[1] >= 2:
                        velocity = (
                            prev_sample_t[:, -1:, :] - prev_sample_t[:, -2:-1, :]
                        ).to(device=device, dtype=x0.dtype)
                    else:
                        velocity = torch.zeros_like(last_motion)
                    steps = torch.arange(
                        1,
                    ...
                    motion_prior = last_motion + steps * velocity
                else:
                    motion_prior = ref_x.to(device=device, dtype=x0.dtype)
                    if motion_prior.ndim == 2:
                        motion_prior = motion_prior.unsqueeze(1)
                    motion_prior = motion_prior[:, :1, :].expand(
                        B, self.num_frames_for_clip, -1
                    )
                x0 = (
                    (1.0 - motion_prior_noise_blend) * x0
                    + motion_prior_noise_blend * motion_prior
                )
            if getattr(self.opt, "debug_session", False) and debug_chunk_index is not None:
                print(
                    ...
                    f"corr={noise_temporal_corr:.3f} "
                    f"prior_blend={motion_prior_noise_blend:.3f} "
