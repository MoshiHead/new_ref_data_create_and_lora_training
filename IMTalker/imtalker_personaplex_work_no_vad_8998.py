        ...
        if reply_engine is not None:
            ...
            def _persona_priority_worker() -> None:
                ...
                while not pipeline_stop_event.is_set():
                    ...
                    for event in events:
                        # Publish native 80 ms audio immediately. Playback still
                        # waits until the shared video/audio epoch is released.
                        reply_b64 = event.get("reply_i16_b64")
                        if reply_b64:
                            reply_pcm = (
                                np.frombuffer(base64.b64decode(reply_b64), dtype=np.int16)
                                .astype(np.float32) / 32768.0
                            )
                            packet = {
                                "audio_packet_index": native_audio_seq,
                                "audio_pcm": reply_pcm.copy(),
                                "created_at": time.perf_counter(),
                            }
                            fut = asyncio.run_coroutine_threadsafe(
                                audio_q.put(packet), event_loop
                            )
                            try:
                                fut.result(timeout=5.0)
                            except Exception as exc:
                                print(
                                    f"[NATIVE-AUDIO] enqueue failed "
                                    f"seq={native_audio_seq}: {exc!r}",
                                    flush=True,
                                )
                            native_audio_seq += 1
            ...
            async def _audio_sender() -> None:
                ...
                while True:
                    ...
                    target_t = send_start_wall + idx * native_packet_sec
